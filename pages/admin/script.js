...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadStats(){
    fetch(`http://127.0.0.1:8000/stats?t=${Date.now()}`, { cache: 'no-store' })
      .then(res => res.json())
      .then(data => {
  ...
  function loadAnalytics(){
    ...
    fetch(`http://127.0.0.1:8000/analytics?t=${Date.now()}`, { cache: 'no-store' })
    .then(res => res.json())
    .then(data => {
      ...
      fetch(`http://127.0.0.1:8000/analytics/monthly?t=${Date.now()}`, { cache: 'no-store' })
        .then(res => res.json())
        .then(mData => {
    ...
    // Daily sales chart
    fetch(`http://127.0.0.1:8000/analytics/daily?t=${Date.now()}`, { cache: 'no-store' })
    .then(res => res.json())
    .then(data => {
      if (!data || data.response !== 'success') return;
  ...
  // Category-wise items pie chart
    fetch(`http://127.0.0.1:8000/analytics/categories?t=${Date.now()}`, { cache: 'no-store' })
    .then(res => res.json())
    .then(data => {
      if (!data || data.response !== 'success') return;
  ...
  // Top customers chart
    fetch(`http://127.0.0.1:8000/analytics/top_customers?t=${Date.now()}`, { cache: 'no-store' })
    .then(res => res.json())
    .then(data => {
      if (!data || data.response !== 'success') return;
